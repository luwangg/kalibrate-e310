...
class circular_buffer
{
    public:
		...
		//
		circular_buffer(size_type capacity = default_capacity, const unsigned int item_size = 1, const unsigned int overwrite = 0)
			: array_(alloc_.allocate(capacity)), array_size_(capacity),
			head_(1), tail_(0), contents_size_(0)
		{
			tmp_ = new value_type[capacity];
		...
		int space_available() {
			...
			int amt;
			pthread_mutex_lock(&m_mutex);
			amt = array_size_ - contents_size_;
			pthread_mutex_unlock(&m_mutex);

			return amt;
		}

		int read(value_type *buf, const int buf_len) {

			int len, idx;

			pthread_mutex_lock(&m_mutex);
			len = MIN_N(buf_len, contents_size_);
		...
		value_type *peek(unsigned int *buf_len) {
		...
		value_type *poke(int *buf_len) {

			int len;
			value_type *p;

			pthread_mutex_lock(&m_mutex);
			len = (array_size_ - contents_size_);
		...
		int write(const value_type *buf, const int buf_len) {

				int len, idx = 0;

				pthread_mutex_lock(&m_mutex);
				
				len = MIN_N(buf_len, (array_size_ - contents_size_));
